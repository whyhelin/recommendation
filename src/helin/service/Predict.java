...
import helin.util.WriteFileUtil;
		...
		for (i = 0; i < pSize; i++) {
			...
			for (j = 0; j < sSize; j++) {
				...
				if (advSim != 0) {
					...
					// have some paths
					if (p.pathCount > 0) {
						advSim = ut.AdvSimilarity(rating, avgs, p, advSim);
					}

					total1 += advSim * (rating[uId][movieId] - avgs[uId]);
			...
			WriteFileUtil.WriteFile("predict.txt", userData + "\t" + movieData
					+ "\t" + (Math.round(preRating) + 0.0) + "\r\n");
