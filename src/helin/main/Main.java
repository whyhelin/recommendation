...
import helin.service.Predict;
import helin.service.SocialGraph;
import helin.service.UserTraining;
 ...
 * Main.java
 * 
 * @description get all essential data(training set, social graph) and predict
 *              unrated movies
 * 
 */
public class Main {
	...
	public static void PredictUnrated() {
		...
		if (new File("res/rating.txt").exists()) {
			...
			try {
				is = new ObjectInputStream(
						new FileInputStream("res/rating.txt"));
			...
			rating = ut.GetTrainingSet("res/training_set.txt", userList,
					movieList);
		...
		System.out.println("read rating matrix time: " + (endTime - startTime)
				+ "ms");
		...
		if (new File("res/graph.txt").exists()) {
			...
			try {
				is = new ObjectInputStream(new FileInputStream("res/graph.txt"));
		...
		predict.PredictUnratedMovies(rating, sg, "res/predict.txt", userList,
				movieList);
