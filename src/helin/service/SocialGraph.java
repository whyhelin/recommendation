package helin.service;

import helin.base.UserBase;
import helin.entity.ArcNode;
import helin.entity.Paths;
import helin.entity.User;
import helin.entity.VertexNode;
import helin.util.Consts;
import helin.util.ReadFileUtil;
 ...
 * SocialGraph.java
 * 
 * @description some operations about users' social graph including creating
 *              graph and get shortest path between all users
 * 
 */
public class SocialGraph implements Serializable {
	 ...
	 * @param vexId2
	 *            another vertex id
	 * @return weight the edge weight if there is directed edge, else a max
	 *         number if not
	 */
	...
	public SocialGraph CreateDG(String sFileName, List<User> userList) {
		...
		// key is one user id, value is a list of all his directly connected
		// user id
		...
		for (int from : socialMap.keySet()) {
			...
			for (i = 0; i < toSize; i++) {
				...
				if (p.adjvex == -1) {
					System.out.println("from = " + from + " and "
							+ toList.get(i) + " has no id.");
					return null;
				}
